...
Failure analysis: After fine-tuning, I'll paste the full list of misclassified test examples into Claude and ask it to identify systematic patterns (e.g., comment length, sarcasm, label pairs that get confused most). I'll verify any claimed pattern myself by re-reading the actual flagged examples — if I can't confirm a pattern by eye, I won't include it in the report as a finding.